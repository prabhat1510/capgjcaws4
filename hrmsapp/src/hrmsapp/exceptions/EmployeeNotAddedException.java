package hrmsapp.exceptions;

public class EmployeeNotAddedException extends Exception{
	String messageException;

	public EmployeeNotAddedException() {
		// TODO Auto-generated constructor stub
	}

	public EmployeeNotAddedException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
		// TODO Auto-generated constructor stub
	}

	public EmployeeNotAddedException(String message, Throwable cause) {
		super(message, cause);
		// TODO Auto-generated constructor stub
	}

	public EmployeeNotAddedException(String message) {
		super(message);
		// TODO Auto-generated constructor stub
	}

	public EmployeeNotAddedException(Throwable cause) {
		super(cause);
		// TODO Auto-generated constructor stub
	}
	
}
